/* ============================================================
   ...
   Themes: base color palette (8 themes)
   Animations: 11 background effects (deterministic, time-based)
   ============================================================ */

const Renderer = (() => {

  /* ══════════════════════════════════════
     SEEDED RANDOMNESS — deterministic arrays
     All animations use only `time` as input
  ══════════════════════════════════════ */
  ...
  /* ══════════════════════════════════════
     THEMES — text colors & base gradient
  ══════════════════════════════════════ */
  const THEMES = {
    classic: {
      base: ['#0d0a2e', '#160840'],
      textActive: '#FFD700', textDim: 'rgba(255,255,255,0.65)', textPrev: 'rgba(255,255,255,0.2)',
      progressBg: 'rgba(255,255,255,0.1)', progressFg: '#7c4dff', shadowActive: 'rgba(255,215,0,0.7)',
    },
    neon: {
      base: ['#050510', '#0a0820'],
      textActive: '#00fff7', textDim: 'rgba(160,220,255,0.7)', textPrev: 'rgba(160,220,255,0.2)',
      progressBg: 'rgba(0,255,247,0.1)', progressFg: '#00fff7', shadowActive: 'rgba(0,255,247,0.9)',
    },
    fire: {
      base: ['#080000', '#220600'],
      textActive: '#ff7700', textDim: 'rgba(255,180,80,0.7)', textPrev: 'rgba(255,180,80,0.2)',
      progressBg: 'rgba(255,100,0,0.12)', progressFg: '#ff6600', shadowActive: 'rgba(255,130,0,0.9)',
    },
    ocean: {
      base: ['#000d1a', '#001c3a'],
      textActive: '#00d4ff', textDim: 'rgba(150,220,255,0.7)', textPrev: 'rgba(150,220,255,0.2)',
      progressBg: 'rgba(0,212,255,0.1)', progressFg: '#00d4ff', shadowActive: 'rgba(0,212,255,0.8)',
    },
    minimal: {
      base: ['#0e0e0e', '#1a1a1a'],
      textActive: '#ffffff', textDim: 'rgba(255,255,255,0.55)', textPrev: 'rgba(255,255,255,0.18)',
      progressBg: 'rgba(255,255,255,0.1)', progressFg: '#fff', shadowActive: 'rgba(255,255,255,0.5)',
    },
    violeta: {
      base: ['#0c0020', '#1a0040'],
      textActive: '#e040fb', textDim: 'rgba(220,150,255,0.7)', textPrev: 'rgba(220,150,255,0.2)',
      progressBg: 'rgba(224,64,251,0.1)', progressFg: '#e040fb', shadowActive: 'rgba(224,64,251,0.9)',
    },
    dorado: {
      base: ['#140e00', '#241600'],
      textActive: '#FFC107', textDim: 'rgba(255,215,90,0.65)', textPrev: 'rgba(255,215,90,0.2)',
      progressBg: 'rgba(255,193,7,0.12)', progressFg: '#FFC107', shadowActive: 'rgba(255,193,7,0.8)',
    },
    hielo: {
      base: ['#ddeeff', '#c0d8f5'],
      textActive: '#0055bb', textDim: 'rgba(0,60,160,0.65)', textPrev: 'rgba(0,60,160,0.2)',
      progressBg: 'rgba(0,80,200,0.12)', progressFg: '#0055bb', shadowActive: 'rgba(0,80,200,0.7)',
    },
  };

  /* ══════════════════════════════════════
     ANIMATIONS — background overlay functions
     (ctx, W, H, t) — t = elapsed seconds
  ══════════════════════════════════════ */
  const ANIMATIONS = {
    ...
    galaxia: (ctx, W, H, t) => {
      ...
      cg.addColorStop(0,   'rgba(220,180,255,0.22)');
      cg.addColorStop(0.3, 'rgba(120,80,220,0.09)');
      ...
      DATA.stars.forEach(s => {
        ...
        const py   = cy + Math.sin(ang) * dist * 0.58;
    ...
    matrix: (ctx, W, H, t) => {
      ...
      const colW = W / DATA.mCols.length;
      const charH = colW * 1.45;
    ...
    aurora: (ctx, W, H, t) => {
      ...
      bands.forEach(b => {
        for (let x = 0; x <= W; x += 3) {
          ...
          const y   = H * 0.28 + w1 * H * 0.14;
          const bH  = H * 0.28 + Math.sin(x / W * Math.PI + t * b.spd * 0.6) * H * 0.06;
          const g   = ctx.createLinearGradient(0, y - bH * 0.4, 0, y + bH * 1.6);
          g.addColorStop(0, 'transparent'); g.addColorStop(0.3, b.c1);
          g.addColorStop(0.65, b.c2);      g.addColorStop(1, 'transparent');
    ...
    particulas: (ctx, W, H, t) => {
      DATA.orbs.forEach(p => {
        const y  = 1 - ((p.y + t * p.spd) % 1);
        const x  = p.x + Math.sin(t * p.wob * 55 + p.ph) * 0.038;
        const px = x * W, py = y * H;
        const glow = ctx.createRadialGradient(px, py, 0, px, py, p.r * 4);
        glow.addColorStop(0,   `hsla(${p.hue},100%,80%,0.55)`);
        glow.addColorStop(0.5, `hsla(${p.hue},100%,60%,0.18)`);
        glow.addColorStop(1,   'transparent');
        ctx.fillStyle = glow;
        ctx.beginPath(); ctx.arc(px, py, p.r * 4, 0, 6.28); ctx.fill();
    ...
    hipnotico: (ctx, W, H, t) => {
      ...
      for (let ring = 1; ring <= 9; ring++) {
        const sides = 3 + ring;
        const rBase = (ring / 10) * Math.min(W, H) * 0.46;
        const pulse = 1 + Math.sin(t * 1.2 + ring * 0.6) * 0.04;
        const r     = rBase * pulse;
        const rot   = t * (ring % 2 === 0 ? 0.28 : -0.22) + ring * 0.55;
        const hue   = (t * 35 + ring * 33) % 360;
        ctx.beginPath();
        ...
        ctx.closePath();
        ctx.strokeStyle = `hsla(${hue},100%,70%,0.28)`;
        ctx.lineWidth = 1.5; ctx.stroke();
      }
      const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, 30);
      g.addColorStop(0, `hsla(${(t*60)%360},100%,80%,0.35)`);
    ...
    confeti: (ctx, W, H, t) => {
      DATA.conf.forEach(c => {
        ...
        const xFrac = c.x + Math.sin(t * 0.7 + c.rot) * 0.04;
        ctx.save();
        ctx.translate(xFrac * W, yFrac * H);
        ctx.rotate(c.rot + t * c.spin);
        ctx.globalAlpha = 0.82;
        ctx.fillStyle = `hsl(${c.hue},92%,65%)`;
    ...
    cosmos: (ctx, W, H, t) => {
      [{ x:0.2,y:0.3,r:0.22,hue:280,ph:0 }, { x:0.76,y:0.6,r:0.19,hue:200,ph:2 }, { x:0.5,y:0.82,r:0.16,hue:320,ph:4 }]
        .forEach(n => {
          ...
          g.addColorStop(0, `hsla(${n.hue},80%,50%,${0.07 + pulse * 0.05})`);
          g.addColorStop(1, 'transparent');
          ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);
        });
      DATA.stars.slice(0, 160).forEach(s => {
        const tw = 0.3 + 0.7 * Math.abs(Math.sin(t * s.spd + s.ph));
        ctx.globalAlpha = s.a * tw * 0.9;
        ctx.fillStyle = '#fff';
        ctx.beginPath(); ctx.arc(s.x*W, s.y*H, s.r*0.7, 0, 6.28); ctx.fill();
      });
      DATA.shoots.forEach(ss => {
        const cycle = (t + ss.startFrac * 13) % ss.interval;
        if (cycle < 1.8) {
          const p = cycle / 1.8;
          const x1 = ss.x0 * W, y1 = ss.y0 * H;
          const len = W * 0.16, ang = Math.PI * 0.26;
          ...
          ctx.strokeStyle = grad; ctx.lineWidth = 2; ctx.globalAlpha = 1 - p * 0.5;
    ...
    destellos: (ctx, W, H, t) => {
      DATA.sparks.forEach(s => {
        ...
        const a   = (pulse - 0.38) / 0.62;
        const hue = (t * 28 + s.ph * 58) % 360;
        ...
        for (let j = 0; j < 8; j++) {
          ...
          const rr  = j % 2 === 0 ? r * 4 : r * 1.2;
          j === 0 ? ctx.moveTo(x + Math.cos(ang)*rr, y + Math.sin(ang)*rr)
    ...
    prisma: (ctx, W, H, t) => {
      DATA.prisms.forEach(pr => {
        const hue   = (pr.hue + t * pr.spdH) % 360;
        const baseX = (pr.baseX + Math.sin(t * 0.1 + pr.ph) * pr.spdX) * W;
        const spr   = pr.spread * W;
        const grad  = ctx.createLinearGradient(baseX, 0, baseX, H * 0.85);
        grad.addColorStop(0,   `hsla(${hue},100%,75%,0.2)`);
        grad.addColorStop(0.5, `hsla(${(hue+40)%360},100%,60%,0.08)`);
        grad.addColorStop(1,   'transparent');
        ctx.beginPath();
        ctx.moveTo(baseX - spr * 0.08, 0);
        ctx.lineTo(baseX + spr * 0.08, 0);
        ctx.lineTo(baseX + spr * 0.9,  H * 0.88);
        ctx.lineTo(baseX - spr * 0.9,  H * 0.88);
        ctx.closePath();
    ...
    olas: (ctx, W, H, t) => {
      for (let layer = 0; layer < 6; layer++) {
        const freq  = 0.5 + layer * 0.35;
        const amp   = H * (0.055 + layer * 0.018);
        const yBase = H * (0.45 + layer * 0.09);
        const spd   = 0.35 + layer * 0.14;
        const hue   = (200 + layer * 22 + t * 7) % 360;
        ctx.beginPath(); ctx.moveTo(0, H);
        for (let x = 0; x <= W; x += 3) {
          const y = yBase
            + Math.sin(x / W * Math.PI * 2 * freq + t * spd) * amp
            + Math.sin(x / W * Math.PI * 3 * freq + t * spd * 0.65 + 1.2) * amp * 0.38;
          ctx.lineTo(x, y);
        }
        ctx.lineTo(W, H); ctx.closePath();
        ctx.fillStyle = `hsla(${hue},78%,42%,0.1)`; ctx.fill();
      }
    },

  };

  /* Exported metadata for UI population */
  ...
  function drawFrame(canvas, opts) {
    const {
      time = 0, duration = 1, lines = [],
      theme = 'classic', animation = 'none',
      fontSize = 56, songTitle = '',
      activeColorOverride, inactiveColorOverride,
    } = opts;
    ...
    /* 2 — Animation overlay */
    ctx.save();
    (ANIMATIONS[animation] || ANIMATIONS.none)(ctx, W, H, time);
    ctx.restore();

    /* 3 — Dim overlay so text always reads */
    ctx.fillStyle = 'rgba(0,0,0,0.28)'; ctx.fillRect(0, 0, W, H);

    /* 4 — Determine active lyric line */
    let activeIdx = -1;
    ...
    const cx   = W / 2;
    const fsLg = Math.max(22, Math.round(fontSize * W / 1920));
    const fsMd = Math.max(16, Math.round(fsLg * 0.62));
    const fsSm = Math.max(12, Math.round(fsLg * 0.44));
    const midY = H * 0.5;

    /* 5 — Song title (top left) */
    if (songTitle) {
      ctx.font = `500 ${fsSm}px 'Segoe UI', sans-serif`;
    ...
    if (pPct > 0) {
      const pg = ctx.createLinearGradient(28, 0, W - 56, 0);
      pg.addColorStop(0, T.progressFg); pg.addColorStop(1, activeColor);
      ctx.fillStyle = pg;
      ctx.beginPath(); ctx.roundRect(28, pbY, (W - 56) * pPct, pbH, pbH / 2); ctx.fill();
    }
    ctx.font = `300 ${fsSm}px monospace`;
    ctx.fillStyle = 'rgba(255,255,255,0.38)';
    ctx.textAlign = 'right'; ctx.textBaseline = 'alphabetic';
    ctx.fillText(`${formatTime(time)} / ${formatTime(duration)}`, W - 28, pbY - 8);

    /* 7 — Previous line (dim, small, above) */
    if (prevLine) {
      ctx.font      = `300 ${fsSm}px 'Segoe UI', sans-serif`;
      ctx.fillStyle = T.textPrev;
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText(_fit(ctx, prevLine.text, W - 80), cx, midY - fsLg * 1.65);
    }

    /* 8 — Next line (medium, below) */
    if (nextLine) {
      ctx.font      = `400 ${fsMd}px 'Segoe UI', sans-serif`;
    ...
    /* 9 — Current line (large, glow + karaoke sweep) */
    if (currLine) {
      const lineEnd = nextLine ? nextLine.time : duration;
      const linePct = lineEnd > currLine.time
        ? clampN((time - currLine.time) / (lineEnd - currLine.time), 0, 1) : 1;

      const txt = _fit(ctx, currLine.text, W - 80);
      ctx.font         = `700 ${fsLg}px 'Segoe UI', sans-serif`;
      ctx.textAlign    = 'center'; ctx.textBaseline = 'middle';

      ctx.shadowColor  = T.shadowActive;
      ctx.shadowBlur   = 28 + Math.sin(time * 4.5) * 7;
      ...
      ctx.save();
      ctx.beginPath();
      ctx.rect(startX - 2, midY - fsLg * 1.1, (txtW + 2) * linePct, fsLg * 2.2);
      ctx.clip();
      ctx.shadowColor = activeColor; ctx.shadowBlur = 14;
      ...
      ctx.font         = `300 ${fsMd}px 'Segoe UI', sans-serif`;
